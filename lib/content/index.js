 ...
 * ####Example:
 *
 *     landmark.content.fetch('home', function(err, content) { ... });
 *
 ...
 * ####Example:
 *
 *     landmark.content.store('home', { title: 'Welcome' }, function(err) { ... });
 *
 * @param {String} key
...
Content.prototype.page = function(key, page) {
	...
	if (arguments.length == 1) {
		...
		if (!this.pages[key]) {
			throw new Error('landmark.content.page() Error: page ' + key + ' cannot be registered more than once.');
		}
...
Content.prototype.initModel = function() {
	...
	var contentSchemaDef = {
		...
		data: { type: landmark.mongoose.Schema.Types.Mixed }
	};

	var ContentSchema = new landmark.mongoose.Schema(contentSchemaDef);

	var PageSchema = new landmark.mongoose.Schema({
	...
	this.AppContent = landmark.mongoose.model('App_Content', PageSchema);
...
Content.prototype.editable = function(user, options) {
	...
	if (!user || !user.canAccessLandmark) {
		return undefined;
	}

	if (options.list) {

		var list = landmark.list(options.list);
