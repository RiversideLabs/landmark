...
function List(key, options) {
	...
	this.options = utils.options({
		schema: {
			collection: landmark.prefixModel(key)
		},
		noedit: false,
		nocreate: false,
		nodelete: false,
	...
	this.schema = new landmark.mongoose.Schema({}, this.options.schema);
	this.uiElements = [];
...
List.prototype.relationship = function(def) {
	...
	Object.defineProperty(def, 'refList', {
		get: function() {
			return landmark.list(def.ref);
		}
	});

	Object.defineProperty(def, 'isValid', {
		get: function() {
			return landmark.list(def.ref) ? true : false;
		}
	});
...
/**
 * Registers the Schema with Mongoose, and the List with Landmark
 *
...
List.prototype.register = function() {
	...
	this.model = landmark.mongoose.model(this.key, this.schema);
 ...
 * ####Example:
 *
 *     list.getSearchFilters('mike') // returns { name: /mike/i }
 *
 * @param {String} query
