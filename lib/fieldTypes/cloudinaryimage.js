...
cloudinaryimage.prototype.addToSchema = function() {
	...
	var folder = function(item) {
		...
		if (landmark.get('cloudinary folders')) {
			if (field.options.folder) {
				folderValue = field.options.folder;
			} else {
				var folderList = landmark.get('cloudinary prefix') ? [landmark.get('cloudinary prefix')] : [];
	...
	var src = function(item, options) {
		...
		if (!('fetch_format' in options) && landmark.get('cloudinary webp') !== false) {
			options.fetch_format = "auto";
		}

		if (!('progressive' in options) && landmark.get('cloudinary progressive') !== false) {
			options.progressive = true;
		}

		if (!('secure' in options) && landmark.get('cloudinary secure')) {
			options.secure = true;
		}
	...
	var schemaMethods = {
		...
		fill: function(width, height, options) {
			return src(this, addSize({ crop: 'fill' }, width, height, options));
		},
		lfill: function(width, height, options) {
			return src(this, addSize({ crop: 'lfill' }, width, height, options));
		},
		...
		crop: function(width, height, options) {
			return src(this, addSize({ crop: 'crop' }, width, height, options));
		},
		thumbnail: function(width, height, options) {
			return src(this, addSize({ crop: 'thumb' }, width, height, options));
		},
		/**
...
cloudinaryimage.prototype.getRequestHandler = function(item, req, paths, callback) {
	...
	return function() {
		...
		if (req.body && req.body[paths.select]) {
			...
			var tp = landmark.get('cloudinary prefix') || '';
			...
			if (landmark.get('cloudinary prefix')) {
				uploadOptions.tags.push(landmark.get('cloudinary prefix'));
			}

			if (landmark.get('env') !== 'production') {
