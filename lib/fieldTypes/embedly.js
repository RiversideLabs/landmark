...
function embedly(list, path, options) {
	...
	if (!landmark.get('embedly api key')) {
		throw new Error('Invalid Configuration\n\n' +
			'Embedly fields (' + list.key + '.' + this.path + ') require the "embedly api key" option to be set.\n\n' +
			'See http://getlandmarkproject.com/docs/cms/configuration/#embedly for more information.\n');
	}
	...
	if (!options.from) {
		throw new Error('Invalid Configuration\n\n' +
			...
			'See http://getlandmarkproject.com/docs/cms/database/#field_embedly for more information.\n');
	}
...
embedly.prototype.addToSchema = function() {
	...
	schema.pre('save', function(next) {
		...
		new EmbedlyAPI({ key: landmark.get('embedly api key') }, function(err, api) {
