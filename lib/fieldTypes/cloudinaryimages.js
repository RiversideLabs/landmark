...
cloudinaryimages.prototype.addToSchema = function() {
	...
	var mongoose = landmark.mongoose;
	...
	var src  = function(img, options) {
		if (landmark.get('cloudinary secure')) {
			options = options || {};
			options.secure = true;
		}
	...
	ImageSchema.method('fill', function(width, height) {
		return src(this, addSize({ crop: 'fill' }, width, height));
	});

	ImageSchema.method('lfill', function(width, height) {
		return src(this, addSize({ crop: 'lfill' }, width, height));
	});
	...
	ImageSchema.method('crop', function(width, height) {
		return src(this, addSize({ crop: 'crop' }, width, height));
	});

	ImageSchema.method('thumbnail', function(width, height) {
		return src(this, addSize({ crop: 'thumb' }, width, height));
	});
