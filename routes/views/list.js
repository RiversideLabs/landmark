...
exports = module.exports = function(req, res) {
	...
	var renderView = function() {
		...
		var link_to = function(params) {
			...
			return '/landmark/' + req.list.path + (p ? '/' + p : '') + (params ? '?' + params : '');
		};
		...
		query.exec(function(err, items) {
			...
			if (req.params.page && items.total && !items.results.length) {
				return res.redirect('/landmark/' + req.list.path);
			}
			...
			if (req.query.search && items.total === 1 && items.results.length === 1) {
				return res.redirect('/landmark/' + req.list.path + '/' + items.results[0].id);
			}
			
			var download_link = '/landmark/download/' + req.list.path,
				downloadParams = {};
			...
			async.eachSeries(req.list.initialFields, compileFields , function() {
				...
				landmark.render(req, res, 'list', _.extend(viewLocals, {
					section: landmark.nav.by.list[req.list.key] || {},
					title: 'Landmark: ' + req.list.plural,
					page: 'list',
					link_to: link_to,
	...
	var checkCSRF = function() {
		var pass = landmark.security.csrf.validate(req);
		if (!pass) {
	...
	if ('update' in req.query) {
		...
		(function() {
			...
			req.list.updateAll(data, function(err) {
				...
				res.redirect('/landmark/' + req.list.path);
			});
		})();
		...
		req.list.model.findById(req.query['delete']).exec(function (err, item) {
			if (err || !item) return res.redirect('/landmark/' + req.list.path);
		...
		item.save(function(err) {
			...
			if (err) {
				...
				return res.redirect('/landmark/' + req.list.path + '/' + item.id);
			}
