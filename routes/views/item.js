		...
		var renderView = function() {
			...
			var relationships = _.values(_.compact(_.map(req.list.relationships, function(i) {
				if (i.isValid) {
				    ...
				    landmark.console.err('Relationship Configuration Error', 'Relationship: ' + i.path + ' on list: ' + req.list.key + ' links to an invalid list: ' + i.ref);
					return null;
				}
			})));
							...
							if (results.length) {
								...
								drilldown.items.push({
									...
									items: _.map(results, function(i) { return {
										...
										href: '/landmark/' + refList.path + '/' + i.id
									};}),
						...
						refList.model.findById(item.get(field.path)).exec(function(err, result) {
							if (result) {
								...
								drilldown.items.push({
									...
									href: '/landmark/' + refList.path + '/' + result.id
								});
							}
							done();
						});
			...
			var loadRelationships = function(cb) {
				...
				async.each(relationships, function(rel, done) {
					...
					rel.list = landmark.list(rel.ref);
			...
			async.parallel([
				...
				landmark.render(req, res, 'item', _.extend(viewLocals, {
					section: landmark.nav.by.list[req.list.key] || {},
					title: 'Landmark: ' + req.list.singular + ': ' + req.list.getDocumentName(item),
					page: 'item',
					list: req.list,
					item: item,
